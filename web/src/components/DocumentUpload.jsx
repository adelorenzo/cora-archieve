...
import { getAcceptString } from '../lib/file-parser';
  ...
  const handleFileSelect = async (event) => {
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        // Process file using client-side parser (supports PDF, DOCX, XLSX, etc.)
        await ragService.processFile(file, (progress) => {
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto space-y-4">
            ...
            <div className="flex gap-2">
              ...
              {(documents.length > 0 || (stats && stats.documents > 0)) && (
                <Button
            ...
            {/* Info */}
            <div className="text-xs text-muted-foreground">
              Supported formats: PDF, Word (.docx), Excel (.xlsx), CSV, Text, Markdown, HTML, JSON
              <br />
              <span className="text-primary">All parsing runs locally in your browser</span>
            </div>
          </div>
        </DialogContent>
      </Dialog>
